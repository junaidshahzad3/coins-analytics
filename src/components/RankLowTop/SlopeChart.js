...
const SlopeChart = ({ filteredData }) => {
  ...
  useEffect(() => {
    ...
    const mcValues = filteredData.flatMap((d) => [
      Number(d["Rank Lowest Low Before"]),
    ...
    filteredData.forEach((d) => {
      g.append("line")
        ...
        .attr("y1", yScale(Number(d["Rank Lowest Low Before"])))
      ...
      g.append("text")
        ...
        .attr("y", yScale(Number(d["Rank Lowest Low Before"])) - 5) // slightly above the point
        .attr("text-anchor", "end")
        .text(d["Coinname + Ticker"]);

      g.append("text")
        .attr("x", xScale("Highest High"))
        .attr("y", yScale(Number(d["Rank Highest High"])) - 5) // slightly above the point
