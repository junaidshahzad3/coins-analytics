...
const SlopeChart = ({ filteredData }) => {
  ...
  useEffect(() => {
    ...
    const mcValues = filteredData.flatMap((d) => [
      Number(d["Rank Lowest Low After"]),
      Number(d["Rank Highest High"]),
    ]);
    ...
    filteredData.forEach((d) => {
      g.append("line")
        ...
        .attr("y1", yScale(Number(d["Rank Lowest Low After"])))
        .attr("y2", yScale(Number(d["Rank Highest High"])))
      ...
      g.append("text")
        ...
        .attr("y", yScale(Number(d["Rank Lowest Low After"])) + 15) // adjusted text position below the point
        .attr("text-anchor", "end")
        .text(d["Coinname + Ticker"]);

      g.append("text")
        .attr("x", xScale("Highest High"))
        .attr("y", yScale(Number(d["Rank Highest High"])) + 15) // adjusted text position below the point
